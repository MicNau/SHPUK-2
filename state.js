...
// Конкретных цен в подписях нет: диапазоны быстро расходились с прайсом, а выбор
// идёт по уровню товара. Отбор на бэкенде — по ТЕГУ категории (PRICE_TIER_TAGS
// в nav-desktop.js), а не по вилке цен.
const PRICE_TIERS = [
  {id:'budget',    lbl:'Эконом'},
  {id:'balanced',  lbl:'Средний'},
  {id:'premium',   lbl:'Премиум'},
  {id:'mpk',       lbl:'МПК'},
];
...
// Ревизия 2026-07-29: бэкенд перешёл на теги dpk / walls / fences.
// 2026-07-31: добавлен тег mpk — террасная доска из МПК (раздел 2329).
// Теги каталога (актуальный список 2026-08-31): walls, fences, dpk, mpk, steps,
// furniture, garden_beds, fencing. Теги terrasnaya_doska и dpk_steps СНЯТЫ бэкендом
// (отбирали по названию товара в обход структуры каталога и давали неверную выдачу);
// запрос с ними отвечает 400. Ни здесь, ни в пресетах ResourceManager их нет.
// section_id с этой же ревизии выборку НЕ сужает (остался для совместимости) —
// фактический отбор идёт по тегу, поэтому тег обязателен каждому разделу.
const SECTION_TAGS = {
...
// Разделы, которые показываются В ОДНОМ списке с основным. Террасная доска бывает из
// ДПК (2314) и из МПК (2329) — это один выбор для пользователя, и фильтр «Доска из МПК»
// обязан находить товары. Раньше запрашивался только раздел элемента (2314), и МПК в
// выдаче не было вовсе — даже при выключенных фильтрах (жалоба 2026-09-01).
// Запрос идёт МУЛЬТИТЕГОМ (`?tags=dpk,mpk`, поддержан бэкендом 2026-08-31), поэтому
// section_id для таких наборов не ставится — он отрезал бы вторую половину.
const SECTION_MERGE = {
  2314: [2329],    // террасная доска: ДПК + МПК
};
...
const S = {
  ...
  bedH: 0.20,
  // Крепёж выбранной грядки (id из BED_MOUNTS) или null. От него зависит МОДЕЛЬ:
  // у грядки с углом свой GLB, с уголком в геометрии (см. planterModelUrl).
  bedMount: null,
