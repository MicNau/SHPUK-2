...
function _nosingPrism(quad, yTop, yBot) {
  // Порядок точек в плане приходит по кромке (наружные, потом внутренние), и обход
  // получается то по часовой, то против — в зависимости от направления ребра. От него
  // зависит навивка треугольников, а значит и куда смотрят нормали: у полуступени они
  // выходили ВНУТРЬ (верхняя грань светилась как изнанка, баг с рендера 2026-09-01).
  // Приводим к одному обходу по знаку площади: при area2 > 0 (обход против часовой в
  // плоскости x-z) верх смотрел бы вниз — разворачиваем.
  let area2 = 0;
...
// Остаток пролёта короче этого в отдельную секцию не выделяем, а добавляем к
// соседней: кусок перил в 15-20 см между двумя столбами читался как сборочный
// брак. Секция-добор, если она есть, ставится У СТЕНЫ дома (см. buildRailing3d).
const RAIL_SECTION_MIN = 0.45;  // минимальная ширина секции-добора, м
