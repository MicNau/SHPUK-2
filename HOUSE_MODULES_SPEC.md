...
Один GLB `mod_railing.glb`, **3 именованных объекта**: `post`, `rails`, `balusters`.

- **Origin**: `(0, 0, 0)` — основание столба, центр сечения. **X** = вдоль ограждения (run),
  **Y** = высота, **Z** = поперёк (толщина, центрировано на Z=0). Следующая секция — на **+1.0 м**
  по X (шаг между осями столбов).
- **Столб (`post`)**: 100×100 мм (X×Z), высота **1200 мм** с навершием (пирамидка сверху).
  На оси x=0. Варианты: сечение 125×125 мм; высота **3000 мм** (опора навеса).
- **Перила (`rails`)**: верхнее и нижнее, сечение **45 (высота) × 90 (поперёк)** мм, длина 1.0 м.
  Верх поручня — 1100 мм (под навершием), низ нижнего перила — 100 мм над настилом.
- **Балясины (`balusters`)**: 50×50 мм, **9 шт на секцию**, шаг **100 мм** (x = 100…900 мм).
  Балясины **2, 5, 8** (x = 200/500/800 мм) идут **от настила** (y=0) до низа поручня; остальные —
  между перилами.
- **Масштаб**: `scale.x` для ширины неполного пролёта (тянутся `rails`/`balusters`); `post` —
  отдельный объект, по X не масштабируется.

**Сборка** (отдельная конструкция, как забор — не часть дескриптора дома): секции набираются
вдоль кромки террасы с шагом ~1 м, на каждой оси — столб. При включённом навесе каждые ~2 м
столб ставится высотой до навеса (вариант 3000 мм), совмещая роль столба ограждения и опоры навеса.

**Интеграция в код (TODO, следующий шаг).** Подключить `mod_railing.glb` в `buildRailing3d`
(`viewer3d-core.js`), заменив процедурные поручень/балясины:
1. Async-загрузка + кэш GLB (как `ensurePlanterLoaded` / GLB-модулей дома); до загрузки —
   текущий процедурный fallback.
2. По сегментам периметра террасы (`terracePerimeterSegments`, с учётом `RAIL_INSET` и проёма
   под лестницу) разложить секции по 1.0 м; остаток пролёта — секция со `scale.x = остаток`.
   `post` (отдельный объект) НЕ масштабировать по X — ставить на каждой оси.
3. Перекрасить `post`/`rails`/`balusters` в `PORCH_COLUMN_COLOR` (единый вид с колоннами навеса);
   высота поручня 1100 мм должна согласоваться с `railH`/`deckHeight`.
4. При включённом навесе (`terrace-roof`) на точках `_terraceColumnPoints` (~каждые 2 м) ставить
   высокий вариант столба (3000 мм / до низа навеса) вместо отдельных колонн — столб ограждения
   служит опорой навеса.
5. Диспоз материалов клонов в `clearGroup` (как у прочих deck/house-мешей); `railingMeshes` для
   примерки материала.
...
| `railing` | Основание столба, центр сечения | секция 1.0 × 1.2 × 0.1 м (9 балясин) | — | X |
