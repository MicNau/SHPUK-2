...
- ❌ **Не реализовано** (можно делать инкрементально): mansard-крыша (наклонные стены 2-го этажа); угловой элемент карниза `mod_cornice_corner.glb` (требуется GLB с трапециевидным сечением — без него на convex-углах карниза остаётся зазор `cd × cd`); CSG-вырез отверстия в скате под velux/dormer (сейчас визуально OK, но без чистой геометрии).
