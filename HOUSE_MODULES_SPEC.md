...
#### Секция забора товара (модель из каталога), соглашение 2026-08-31

- **`fence_post`** — столб; секция считается по ОСЯМ столбов, шаг 2 м. Столб в файле
  один (в начале секции): конечный принадлежит следующей секции.
- **`fence_left` / `fence_right`** — боковины секции. Ими ОГРАНИЧИВАЕТСЯ полотно:
  доски и плетёнка в файле обычно нарисованы лентой с запасом, и во viewer3d она
  режется по диапазону боковин (`_fenceProtoLimits` + `_fenceClipX`).
- Боковины — каркас: красятся тёмно-серым вместе со столбами и прогонами, товаром
  красится только полотно.
