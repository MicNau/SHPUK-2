...
function buildGableRoof(parent, baseY, bbox, angleDeg, eave, sharedWallMat, skipGableSides, noRakeOverhang) {
  ...
  function buildSlopeBox(topCorners, normal) {
    ...
    const sideIdx = [
      ...
      1, 2, 6, 1, 6, 5,
      // ВНИМАНИЕ: торец у конька не строим. Два ската сходятся здесь вплотную,
      // их торцевые плиты налезали друг на друга и мерцали (полигонфайтинг на
      // коньке). Грань всё равно не видна — она внутри стыка.
      // 2, 3, 7, 2, 7, 6,
