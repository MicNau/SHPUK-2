...
### Canvas-редакторы:
Каждая секция с редактором (terrace, steps, paths, fence, beds, facade, pool_terrace, pier) имеет свой
...
1. **Бэкенд** — FastAPI + расчётный модуль + БД (у команды бэкенда, не начат).
2. **`mod_cornice_concave_corner.glb`** — L-образный добор для стыка карнизов на
   вогнутых углах (сейчас они просто перекрываются на 15 см, что незаметно).
3. **Пересборка GLB дверей** — handle сделать sibling `leaf_main` (не child), чтобы
   ручка не масштабировалась вместе со створкой.
4. **Разделы каталога без товаров** на бэкенде (ступени, грядки, ограждения, мебель,
