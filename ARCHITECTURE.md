...
спецификацию материалов и работы. Клиентский `_computeEstimate` остаётся для
остальных элементов (своих ручек у них пока нет), поэтому в «Итоге» две таблицы:
ориентировочная смета и точный расчёт террасы под ней.
