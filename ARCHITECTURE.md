...
- **Садовая мебель:** точки размещения на плане (поставить/подвинуть/удалить, нумерованные); товар из каталога назначается точке (выбранной или первой свободной по номерам) и спавнится в 3D — на настиле террасы, если точка на ней, иначе на земле. Модель берётся из поля товара (`modelUrl`), пока его нет — локальный фолбэк. См. «Садовая мебель» ниже.
...
const S = {
  ...
  furniture:    [{ x, y, product }], // точки садовой мебели (номер = индекс+1), product | null
...
**Поток данных.** Пользователь ставит на плане точки (`S.furniture = [{x, y, product}]`,
номер точки = индекс+1), затем назначает им товары из каталога. Одна точка — один
предмет мебели.

| Слой | Что делает | Где |
|------|-----------|-----|
| План-редактор | постановка/выбор/перетаскивание/удаление точек, снап 0.5 м | `initFurnitureCanvas` / `drawFurnitureCanvas`, canvas.js |
| Назначение товара | «Применить» → выбранная точка, иначе первая свободная ПО ПОРЯДКУ НОМЕРОВ | `_assignFurnitureProduct`, nav-desktop.js |
| Источник модели | поле товара → локальный фолбэк | `furnitureModelUrl`, viewer3d-builders.js |
| Загрузка/кэш | GLTFLoader, кэш по URL, пересборка сцены по готовности | `ensureFurnitureModel`, viewer3d-builders.js |
| Расстановка | центр по точке, основание на отметку поверхности | `buildFurniture3d` + `surfaceYAt` в `buildScene3d` |
...
**Контракт с бэкендом (ожидается).** Фронт читает URL модели товара из
`ProductResource.modelUrl`, который заполняется из **`model_url`** ИЛИ
**`texture_urls.model_glb`** (проверяются оба, см. ResourceManager.js). Пока поля нет —
подставляется локальный фолбэк по названию товара: «лампа/светильник/фонарь/торшер» →
`mod_lamp_a.glb`, иначе `mod_bench_a.glb`.
...
- модель отдаётся в НАТУРАЛЬНУЮ величину — код её НЕ масштабирует (в отличие от
  растительности): диван 2 м останется 2 м;
- разумный полигонаж (до ~20 тыс. треугольников) — моделей в сцене может быть много.

**Когда модели появятся на бэкенде** — порядок проверки:
1. `GET /api/v1/products/?section_id=2442` (или тег мебели) — убедиться, что у товара
   есть `model_url` / `texture_urls.model_glb`;
2. если имя поля иное — добавить его в `ProductResource` (ResourceManager.js, две
   строки: конструктор и `updateByData`);
3. если раздел/тег мебели новый — прописать в `CATALOG_SECTIONS`,
   `CONSTRUCTION_TO_SECTION.furniture` и `SECTION_TAGS` (state.js);
4. проверить CORS/доступность .glb с фронта (текстуры уже ходят с `Access-Control-Allow-Origin: *`,
   но модели лежат по другому пути) и посадку модели на террасе и на земле.
...
1. **Модели мебели с бэкенда** — ожидаются (обещаны 2026-08-02). Код готов: как только у товара
   появится `model_url` / `texture_urls.model_glb`, модели подхватятся без правок. Порядок
   проверки — в разделе «Садовая мебель». Пока работает локальный фолбэк.
