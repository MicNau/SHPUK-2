...
2. **Архитектура параметрической модели дома** — обсудить подход: процедурная оболочка (стены, крыша, цоколь) + модульные GLB-детали из Blender (окна, двери, элементы декора)
3. **Бэкенд** — FastAPI + расчётный модуль + БД
4. **Десктопная версия UI** — другой layout, но тот же viewer3d-core
