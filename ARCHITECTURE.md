...
- `buildPaths3d()` / `_buildPathRibbon()`, `buildFence3d()` — поддерживают multi-line через `splitAtBreaks()`. Дорожка — монолитная лента с миттером кромок на углах (`_offsetPolyline`); доски идут **поперёк** ленты (UV повёрнут на 90°: `U` — поперёк, `V` — вдоль).
...
- **Терраса и ступени** получают свою подкладку под ними (`buildConstructionPad` в
  ...
  deck-материала не перекрашивала отмостку.
