...
## Статус: фронтенд разбит на файлы, PBR-визуализация работает, вальмовая крыша + входная зона реализованы, GLB-растительность готова (ожидают ассеты), бэкенд не начат
...
- `buildHouseMeshes()` — стены с окнами/дверью, цоколь, вальмовая (hip) крыша, входная зона с колоннами и деревянной обшивкой.
...
- Вальмовая крыша: 4 ската (2 трапеции + 2 треугольных вальма), конёк ~55% длины дома, свес 0.45 м. Лобовая доска по периметру.
- Входная зона: крытое крыльцо с 2 колоннами, деревянная обшивка задней стенки (woodClad), боковые стенки, козырёк (продолжение крыши), ступени и площадка.
...
|----------|-----|---------------------|----|-------------|
| `wall` — штукатурка | MeshStandardMaterial | wall_diff/norm/roug | кубическая, 2 м/тайл | цвет 0xf2f2ee (белый) |
| `base` — цоколь | MeshStandardMaterial | base_diff/norm | кубическая, 1 м/тайл | цвет 0x3a3a3c (тёмный антрацит) |
| `roof` — крыша | MeshStandardMaterial | roof_diff/norm/roug | по скату, 8 м/тайл | цвет 0x404045 (тёмно-серый) |
| `glass` — стекло | MeshStandardMaterial | — | — | opacity 0.38, metalness 0.82, color 0x4a6878 |
| `frame` — рамы | MeshStandardMaterial | — | — | цвет 0x3a3a3c (антрацит), polygonOffset: -1/-1 |
| `door` — дверь | MeshStandardMaterial | — | — | цвет 0x2a2a2e (тёмный) |
| `woodClad` — дерев. обшивка | MeshStandardMaterial | — | — | цвет 0xB08050 (тёплое дерево) |
| `column` — колонны | MeshStandardMaterial | — | — | цвет 0xf0f0ec (белый) |
...
| Вальмовая крыша (4 ската) | Референс: одноэтажный дом с hip roof, все 4 стороны со скатами |
| Тёмные рамы (антрацит) | Референс: тёмные оконные профили вместо белых |
| Тёмный цоколь (антрацит) | Референс: контрастный тёмный цоколь |
| Белые стены | Референс: чистая белая штукатурка |
| Входная зона с колоннами | Референс: крытое крыльцо с 2 колоннами, деревянной обшивкой, козырьком |
| Дверь 1.6×2.3 м (стеклянная) | Референс: широкая стеклянная входная дверь |
| Свес крыши 0.45 м | Референс: заметный выступ крыши за стены |
...
1. **Подготовить GLB-модели растительности** — экспорт из Blender по соглашению выше, положить в assets/
2. **Доработка модели дома** — детализация входной зоны (освещение, ручка двери, водосток), дымоход, отмостка
3. **Параметризация типа крыши** — переключение между вальмовой / двускатной / плоской
4. **Бэкенд** — FastAPI + расчётный модуль + БД
5. **Десктопная версия UI** — другой layout, но тот же viewer3d-core
