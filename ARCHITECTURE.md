...
/frontend — общие файлы
  ...
  backend_API/            # выгрузка бэкендера, правится на их стороне:
                          ...
                          #   readme.txt          — справочник каталожного API;
...
Ручек расчёта шесть, по одной на тип объекта (контракт — `backend_API/calculation_api.md`,
выложен 2026-08-18):
...
**Подключена пока только терраса.** Ступени, ограждения, дорожки, забор и мебель
по-прежнему считает клиентский `_computeEstimate`, поэтому в «Итоге» две таблицы:
смета по каталогу и полный расчёт террасы под ней. Перевод остальных элементов на
серверный расчёт требует под каждый тип собрать геометрию в миллиметрах с разметкой
вершин — так же, как это сделано для террасы.
...
**Смета всегда по одному объекту**: тело запроса — то же, что у расчёта этого типа,
общей сметы на проект контракт не предусматривает. Поэтому кнопка «Смета террасы в PDF»
живёт в блоке расчёта террасы (`_dRenderTerraceCalc` → `dTerraceReport`), а не в окне
«Итог». Ссылка приходит относительной, поэтому разворачивается по `RESOURCE_API_DOMAIN`
(на статике API и страница — разные хосты).
