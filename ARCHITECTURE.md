...
- **Бэкенд (свой):** не начат. Каталожный API — внешний (предоставлен; требует включения CORS для продакшна).

## Запуск (dev)
- **Сервер:** `python devserver.py [порт]` (по умолчанию 8848) или двойной клик по `run-server.bat`. Раздаёт сайт + проксирует `/api` и `/static` на `sollersdev.ru` (same-origin, без CORS). Открыть: `http://localhost:8848`.
- **Важно:** просто открыть `index.html` файлом нельзя — каталог не загрузится (нужен прокси). Конфиг домена API — `RESOURCE_API_DOMAIN` в `index.html` (`''` = через прокси; `'https://sollersdev.ru'` = напрямую, когда апстрим включит CORS).
...
- **Терраса и ступени** получают свою подкладку под ними (`buildConstructionPad` в
  `viewer3d-core.js`, материал `mat_construction_pad`): axis-aligned footprint конструкции
...
- **Каталожный API подключён** (`ResourceManager.js`, версия `?v=1`). Это клиент к боевому REST-API `https://sollersdev.ru/api/v1` (разделы, продукты с ценами и **PBR-текстурами** diffusion/normal/roughness). Файл — плоские глобалы (`ResourceManager`, `Filter`, `FilterType`, `Presets`, `ProductResource`, `SORT_FILEDS`, `SORT_ORDER`), зависит от `THREE` (грузится после three.min.js, до nav-desktop.js).
  - **Домен API настраивается** глобалом `RESOURCE_API_DOMAIN` (в `index.html`). `''` = same-origin (через dev-прокси), `'https://sollersdev.ru'` = напрямую (когда бэкенд включит CORS). Это единственная правка в `ResourceManager.js` относительно оригинала.
  ...
  - **Известно/TODO:** грядёт `trade_offers` (вариации товара) — структура продукта изменится (цены/текстуры/превью переедут внутрь), не завязываться жёстко. `preview_picture.url` пока null. Цены в ₽/м.пог, а тиры фильтра — ₽/м² (рассинхрон единиц). Цвето-фильтр на реальные товары не влияет (нет поля цвета). Текстуры только у ДПК-досок.
...
5. **Доработки модульной системы (некритично, инкрементально):**
   ...
   - ~~Dormer/velux на скате крыши~~ ✅ (`buildRoofWindows` в `shared/house-builder.js`, velux через GLB в плоскости ската + custom flat glass; dormer процедурный с правосторонним базисом и автоматическим утоплением). У обоих стекло некалёное + **полигон-штора** (`mat_curtain`, белый + карта нормалей) за стеклом. Вынос (глубина) dormer'а ограничен горизонтальным пробегом ската на его позиции (`d ≤ 2·min(posUp,1−posUp)·horizRun − запас`) → масштабируется с домом, не торчит за карниз на маленьком доме.
