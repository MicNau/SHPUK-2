...
Шаг секции — 2 м по осям столбов, высота столба 1.9 м (`FENCE_NATIVE_H`). Локальный
+X кладётся вдоль пролёта; неполный остаток пролёта получает секцию, ужатую по X
(`kx` в `_placeFenceSection`, столб при этом не сжимается). В конце пролёта ставится
секция «только столб» — если такого столба ещё нет в реестре (углы общие у соседних
пролётов). Высота полотна `S.fenceH` (1.5/1.9) применяется масштабом по Y.
