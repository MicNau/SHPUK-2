...
viewer3d-core.js?v=136    viewer3d-builders.js?v=23   viewer3d-railing.js?v=7
...
**Материалы назначаем сами, UV берём из GLB.** Товаром красится ТОЛЬКО полотно; столбы и
вспомогательные элементы (меши/материалы с именем по `FENCE_FRAME_RE`) — всегда тёмно-серые
`FENCE_FRAME_COLOR`. Развёртка досок при этом не трогается: её делают в самом GLB (решение
