...
```
styles-desktop.css?v=35   state.js?v=56              canvas.js?v=52
shared/house-builder.js?v=85                          ResourceManager.js?v=6
viewer3d-core.js?v=142    viewer3d-builders.js?v=33   viewer3d-railing.js?v=10
viewer3d-entourage.js?v=14                            nav-desktop.js?v=97
...
Свободный конец пролёта закрывается столбом отдельно — но только если модель сама его туда
не ставит. `_fenceProtoPosts` разбирает прототип и запоминает, есть ли столб в начале секции
(X≈0) и в её конце (X≈nativeW); столбом считается меш, узкий вдоль пролёта и высокий (или
прямо названный столбом, `FENCE_POST_RE`), и не опознанный как полотно. Если столб в конце
секции есть — последняя секция уже закрыта, добавлять нечего. Если только в начале —
`_fenceModelPost` клонирует секцию и оставляет в ней ТОЛЬКО этот столб: всё остальное
(полотно, горизонтальные прогоны во всю секцию, крепёж) выбрасывается. Раньше выбрасывалось
одно полотно, и на свободном конце вместе со столбом висели прогоны длиной в секцию.
`_fenceBoxPost` — фолбэк для условного вида и для моделей, в которых столб не распознан.
Все ветки регистрируют столб в общем `postSet`, поэтому на стыке пролётов дубля нет.
