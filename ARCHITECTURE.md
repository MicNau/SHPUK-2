...
- **Конструкции:** терраса/крыльцо (multi-rect) с **ограждением** (GLB-модуль `mod_railing`, единый контур объединения блоков) и навесом, ступени, дорожки (монолитная лента), **забор** (GLB-модули `mod_fence_00N`, выбор типа превьюшками, шаг секции 2 м, высота полотна 1.5/1.9 м — `S.fenceH`), **грядки** (GLB-плантер 3×1; до выбора товара — габаритный прямоугольник). Растительность сейчас **отключена** (легко вернуть — см. ниже).
...
const S = {
  ...
  bedH:         0.20,                  // высота борта (м); в редакторе НЕ задаётся —
                                       // приходит с карточки товара (TODO.md → ГРЯДКИ)
...
### Забор — сборка из GLB-модулей

Модули: `assets/houses/modules/fences/mod_fence_001|002|003.glb`, тип выбирается в
редакторе забора превьюшками (`S.fenceType`, `FENCE_TYPES` в viewer3d-builders.js).

**Устройство модуля** (у всех трёх габариты совпадают, отличается профиль планок и
наличие среднего стояка):

| Часть | Диапазон по X | Что делает код |
|-------|---------------|----------------|
| `fence_post` | −0.114…0.114 (центр на 0) | столб; НЕ тянется по X, дедуплицируется на стыках |
| `Fence_panels` | 0…2 | **планки — единственная текстурируемая часть** (TODO.md) |
| `fence_bottom` / `fence_top` | 0…2 | лаги, тянутся вместе с полотном |
| `fence_middle` | ≈1 (только 002/003) | средний стояк |

Высота столба — 1.9 м (`FENCE_NATIVE_H`), высота полотна `S.fenceH` (1.5/1.9) даётся
масштабом по Y всей секции. В конце пролёта ставится секция «только столб» — если такого
столба ещё нет в реестре (углы общие у соседних пролётов).

**Секции в пролёте — РАВНОЙ ширины**: `n = max(1, round(длина / 2))`, ширина = длина / n.
Схема «целые по 2 м + остаток» прижимала к углу узкую секцию (на типовом периметре —
0.3 м) со сплющенными планками; при округлении ширина держится в пределах ~1.5–2 м и
искажение планок незаметно.

**⚠ Ось растяжения секции.** У частей модуля СВОЙ поворот: вдоль пролёта (родительский
+X) смотрит локальная **Z** у планок и лаг, **Y** у среднего стояка и только у столба —
**X**. Масштаб применяется ДО поворота (`M = T·R·S`), поэтому тянуть надо ту локальную
ось, которая ПОСЛЕ поворота легла вдоль пролёта (`_localAxisAlongSpan`). Наивное
`o.scale.x *= kx` сжимало планки по ВЫСОТЕ, а по длине они оставались 2 м и вылезали
за угол — та же грабля, что с центральной секцией водостока (v=124). `position.x`
при этом масштабируется как есть: позиция задана в системе родителя. Столб и средний
стояк сохраняют сечение — им меняется только положение.

**Материалы.** Планки красит выбранный товар — забор входит в `DECK_MAT_ELEMENTS`,
поэтому у него свой `S.elementMat.fence` с текстурами. Остальные части — сплошной цвет
`_fenceFrameColor`: имя цвета товара (`colorName` → `CATALOG_COLOR_HEX`) → средний цвет
текстуры планок (`_avgTextureColor`, канвас 8×8; `SecurityError` на чужом origin ловится)
→ нейтральное дерево `FENCE_FRAME_FALLBACK`.
...
- При нажатии «Применить» материал добавляется в образцы, применяется к 3D-сцене и пишется
  в `S.estimate[элемент]` (`_setEstimateForActive`); смета и расчёт террасы пересчитываются
  при следующем открытии «Итога».
...
Сделано в итерации v=132 (смета обновляется по «Применить»):

- **Кнопка «В смету» на карточках товара убрана** (`dEstimateMat`/`dEstimateRealMat`/`_estimateToast`
  и стиль `.d-mat-btn-estimate` удалены): применённый материал И ЕСТЬ выбор для сметы. Два действия
  на карточке позволяли развести материал в 3D и материал в смете — источник расхождений.
- **`_applySampleToActive` пишет строку сметы** (`_setEstimateForActive`): цена прокинута в образец
  (`price`), поэтому повторное применение свотча обновляет смету так же, как карточка. Если у
  источника поля `price` нет вовсе — строка сметы НЕ трогается (иначе повтор свотча её стирал бы).
- Cache-bust: `styles-desktop.css?v=8`, `nav-desktop.js?v=59`.

Сделано в итерации v=131 (фикс погонажа в смете):

- **`_boardWidthM` не читал ширину доски** и почти всегда возвращал фолбэк 0.14 м: регулярка
  брала ПЕРВОЕ число пары, а в каталоге название вида «20*140*2900» начинается с толщины.
  У террасной доски фолбэк совпадал с реальными 140 мм, поэтому ошибку не было видно; ступени
  (20х320х2900) и фасадные панели (13х177х3600) считались с завышением погонажа в 2.3 и 1.3 раза.
- **Порядок размеров в каталоге не единый** («доска 20*140*2900» — толщина×ширина×длина, но
  «ступень узкая 150*25*3000» — ширина×толщина×длина), поэтому позиция числа ненадёжна: из
  группы размеров выбрасываем длину (≥ 1000 мм), ширина = БОЛЬШЕЕ из оставшихся (толщина ДПК —
  12–25 мм). Диапазон валидации расширен до 80–400 мм (ступень 320 мм в старый предел 300 не влезала).
- Cache-bust: `nav-desktop.js?v=57`.

Сделано в итерации v=130 (расчёт террасы на бэкенде):

- **`POST /api/v1/calculate_terrace/`** подключён: спецификация материалов + работы
  показываются в «Итоге» отдельной таблицей под ориентировочной сметой. Полный контракт,
  соглашения по координатам и открытый вопрос по знаку `doorDirection` — в разделе
  «Расчёт террасы».
- **Контур террасы для расчёта** берётся из `_terraceUnionLoops` (тот же, что у ограждения),
  но в метрах плана, а не в мировых координатах, — расчёт не зависит от 3D-сцены.
  Вершины на стене дома помечаются `vertexType: 'house'` (проверено на прижатой,
  отдельно стоящей и Г-образной террасе).
- **`S.elementMat[el]` теперь хранит `productId`/`name`** применённого товара — по нему
  расчёт узнаёт выбранную доску. 3D-слой эти поля не читает.
- **`devserver.py` научился проксировать POST** (`do_POST` + тело/Content-Type; тело ошибок
  апстрима больше не глотается) — раньше локально POST падал в 501 от SimpleHTTPRequestHandler.
- **`getHouseFacadeLayout`** прокидывает флаг `main` у двери — по нему определяется
  главный вход для `doorDirection`.
- Cache-bust: `shared/house-builder.js?v=82`, `nav-desktop.js?v=56`.

Сделано в итерации v=129 (материал рам + превью в карточках товаров):

- **«Материал рам»** — четвёртая группа образцов на шаге «Параметры дома»: дерево (текущий `#4a2f18`),
  белый (`#f2f2f0`), тёмно-коричневый (`#2b1a0d`). Значение в `S.frameMat`, применяется в
  `_applyHouseMaterials` к мешам `mat_frame*`/`mat_door` (раньше там был захардкожен
  `HOUSE_WOOD_COLOR`). Цвета продублированы в `HOUSE_MATERIALS.frame` (образцы UI) и в
  `_houseTexSet('frame')` (3D) — держать в синхроне.
- **UI материалов дома стал data-driven**: `_dRenderHouseMaterials`/`dSetHouseMat` идут по ключам
  `HOUSE_MATERIALS` и пишут в `S['<kind>Mat']`, а не по списку `['roof','base','wall']` с if-цепочкой.
  Новая группа материалов теперь не требует правок в nav-desktop.js.
- **Превью в карточках товаров**: миниатюра берётся по цепочке DPK-текстура → `preview_picture` →
  `detail_picture` → серый квадрат (`_productThumbStyle` + нормализатор `_pictureUrl`: строка,
  объект `{src|url|path}`, числовой id отбрасывается). Раньше источник был один — DPK-текстура,
  поэтому у мебели (текстур нет, есть фото каталога) карточки шли серыми.
- Cache-bust: `state.js?v=34`, `viewer3d-core.js?v=121`, `nav-desktop.js?v=55`.
