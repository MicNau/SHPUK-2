...
|---|---|---|
| Терраса | `{vertices, doorDirection, deckingBoardProductId}` | `buildTerraceCalcRequest` (union-контур блоков) |
...
**Что в проект не попадает.** Терраса у бассейна и причал — своих типов расчёта
в API нет. Дорожки — API ждёт замкнутый контур ленты с отмеченной стартовой
стороной, а у нас хранится осевая линия с шириной; нужен построитель контура.
Ступени без примыкания к террасе тоже пропускаются: такой расчёт заведомо падает
