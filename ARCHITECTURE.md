...
высокий (или прямо названный столбом, `FENCE_POST_RE`), не опознанный как полотно. Оттуда
берутся центры крайних столбов и **шаг** `pitch = endCx − startCx` — по нему и ставятся секции,
а НЕ по габариту модели: в габарит входит по полстолба с каждого края, и секции, поставленные
по нему, давали на каждом стыке два столба вплотную (на углу — четыре). Центр стартового столба
совмещается с началом секции, конечный столб из клона выбрасывается — его место занимает
стартовый столб следующей секции. Столбы контр-масштабируются по X, чтобы не менять толщину
вместе с длиной пролёта.
