...
- **Лестница: ньюэлы + inset перил** (v=65). Перила лестницы сдвинуты к центру (`STAIR_RAIL_INSET = 0.12`, `latOff = stairWidth/2 − inset`) — не конфликтуют с углом террасы. Добавлены вертикальные ньюэл-стойки вверху (`bh`→поручень) и внизу (земля→поручень) — наклонный поручень входит в стойки.
...
- Cache-bust: `canvas.js?v=21`, `viewer3d-core.js?v=65`, `shared/house-builder.js?v=63`.
