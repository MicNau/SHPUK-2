...
- **Материалы:** каждый деко-элемент (терраса/ступени/дорожки/грядки/забор/**ограждение**) красится **независимо** (`S.elementMat` — список в `DECK_MAT_ELEMENTS`); терраса у бассейна и причал материала своего НЕ имеют — у них общий с террасой ключ (`MAT_ALIAS`/`matKey` в state.js), товар применяется сразу ко всем трём; применённый товар он же выбор для сметы. PBR-текстуры товара ложатся на deck-материал с кубическим UV (ребро 1 м, `UV_TILE` — единое для всей сцены); исключение — ограждение, у него UV приходят из GLB-модуля.
...
- **Материал у трёх террас общий** (`MAT_ALIAS`/`matKey`): товар, применённый к бассейну или
  причалу, применяется и к террасе — это одна доска, раздельный выбор выглядел бы ошибкой.
  Через `matKey` идут все чтения/записи `S.elementMat` и `S.estimate`.
