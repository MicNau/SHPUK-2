...
- **Конструкции:** терраса/крыльцо (multi-rect; высота настила настраивается 15 см…фундамент), **ограждение террасы** — ОТДЕЛЬНЫЙ элемент проекта: рисуется ломаной (точки прилипают к кромкам настила и стенам дома с отступом в полсечения столба), строится из GLB-модуля `mod_railing`; ступени, дорожки (монолитная лента), **забор** (условный вид до появления модели у товара; шаг секции 2 м, высота полотна 1.5/1.9 м — `S.fenceH`), **грядки** (GLB-плантер 3×1; до выбора товара — габаритный прямоугольник, после — высота борта из товара). Растительность сейчас **отключена** (легко вернуть — см. ниже).
- **Материалы:** каждый деко-элемент (терраса/ступени/дорожки/грядки/забор/**ограждение**/бассейн) красится **независимо** (`S.elementMat` — список в `DECK_MAT_ELEMENTS`); применённый товар он же выбор для сметы. Терраса у бассейна берёт ТОТ ЖЕ набор карточек, что терраса (раздел 2314 + палитра `terrace`), но доска у каждой своя и строка сметы отдельная. PBR-текстуры товара ложатся на deck-материал с кубическим UV (ребро 1 м, `UV_TILE` — единое для всей сцены); исключение — ограждение, у него UV приходят из GLB-модуля.
...
/frontend — общие файлы
  ...
  canvas.js               # pan/zoom движок, snap-canvas (дорожки/забор/ограждение),
                          # прямоугольный редактор (терраса, бассейн — initRectCanvas),
...
```
styles-desktop.css?v=31   state.js?v=49              canvas.js?v=47
shared/house-builder.js?v=85                          ResourceManager.js?v=6
viewer3d-core.js?v=137    viewer3d-builders.js?v=24   viewer3d-railing.js?v=8
viewer3d-entourage.js?v=14                            nav-desktop.js?v=87
...
const S = {
  ...
  // бассейна — ОТДЕЛЬНО СТОЯЩАЯ (снап к сетке и своим блокам, все 'free').
...
конструкций, **[railing]** — периметр и ограждение.
...
- `_applyDeckUV(mesh, plankAlongX, uvOffset)` — то же с ориентацией досок; `uvOffset`
  позволяет уйти с мировой сетки на локальную привязку. Так сделаны **проступи лестницы**:
  поперёк ступеней UV отсчитываются от ЦЕНТРА проступи, поэтому шов доски приходится ровно
  на середину ступени. Мировая проекция там не работала: доска ≈ 0.11 м, проступь 0.29 м,
  шаг лестницы 0.28 м — периоды несоизмеримы, и шов попадал то на нос, то на пятку, на
  каждой ступени по-разному. ВДОЛЬ ступени проекция осталась мировой.
...
- `buildTerrace3d()` **[builders]** — настил из досок + лаги + опоры по произвольному полигону + юбка (skirt panels, deck-материал, толщина 0.06). Высота настила = `foundH - 0.01` (на 1 см ниже цоколя, чтобы избежать z-fighting). Полигональных секций в UI больше нет (терраса у бассейна переведена на прямоугольники), функция остаётся как построитель по контуру.
- `_buildRectDecks(polys, deckH, hEdges)` **[core, локальная в buildScene3d]** — настил ПРЯМОУГОЛЬНЫХ секций: терраса/крыльцо (`hEdges` = рёбра дома) и терраса у бассейна (`hEdges = null` → доски вдоль длинной стороны блока). Внутри — подкладки, миттер углов и `_buildTerracePoly` на каждое крыло.
...
- Типы зон: `rect` (дом, крыльцо), `poly` (терраса, терраса у бассейна), `path` (дорожки с шириной).
...
или террасы у бассейна → настил (`bh − 0.01`); иначе земля (0).
...
|---|---|---|
| Терраса, терраса у бассейна | `{vertices, doorDirection, deckingBoardProductId, terraceHeight}` | `buildTerraceCalcRequest(secId)` (union-контур блоков секции). У отдельно стоящей все вершины `free`, дом в расчёте не участвует |
...
**Материалы назначаем сами, UV берём из GLB.** Товаром красится ТОЛЬКО полотно (меши/материалы
с именем по `FENCE_PANEL_RE`); ВСЁ остальное — столбы, каркас, крепёж, добор — тёмно-серое
`FENCE_FRAME_COLOR`, roughness 0.6. Правило именно такое, «по умолчанию серое»: раньше искали
каркас списком имён, и любая непонятная деталь модели уезжала в текстуру товара. Если в модели
не нашлось ни одного полотна, забор выйдет целиком серым — на это есть `console.warn`, чтобы на
стенде сразу дополнить `FENCE_PANEL_RE` именами из конкретного файла.

Развёртка досок при этом не трогается: её делают в самом GLB (решение
продукта 2026-08-23), поэтому вертикальный и горизонтальный забор различаются только файлом
модели — признака ориентации в карточке товара не нужно. Единственное исключение — меш вовсе
без UV: для него остаётся аварийная box-проекция, иначе текстура легла бы одним пикселем.
В `threeState.fenceMeshes` (по нему идёт примерка образца цветом) попадает только полотно.

Пока у товара нет модели, забор строится в **условном виде**: столбы + полотно
(`FENCE_SCHEMATIC_COLOR`, либо текстура выбранного товара, если она есть). Это осознанно
«черновой» вид — разметка видна в сцене и считается в смете, но никто не примет её за
реальный профиль. UV полотна здесь наши: box-проекция вдоль длинной стороны панели
(`_applyFenceUV`) — авторской развёртки взять неоткуда.
...
применяется масштабом от `FENCE_NATIVE_H`. Секция модели тянется по X до ширины пролёта, и
UV тянутся вместе с геометрией — доски становятся шире/уже вместе с самой панелью, рисунок
с ней не расходится.

Свободный конец пролёта закрывается столбом отдельно: у модели столб стоит в НАЧАЛЕ секции.
`_fenceModelPost` клонирует секцию и оставляет в ней только каркасные меши, `_fenceBoxPost` —
фолбэк для условного вида и для моделей без распознанного каркаса. Обе ветки регистрируют
столб в общем `postSet`, поэтому на стыке пролётов дубля нет.
...
- Порядок пунктов (`D_SIDEBAR_ITEMS`): терраса → **ограждения террасы** → ступени → дорожки →
  забор → грядки → садовая мебель → бассейн. («Отделка фасада» временно скрыта из меню,
  см. `hidden` в `D_SIDEBAR_ITEMS`; причал убран совсем.)
...
### Canvas-редакторы:
Каждая секция с редактором (terrace, railing, steps, paths, fence, beds, facade, furniture, pool_terrace) имеет свой
`d-center-canvas` overlay: обёртка `cw-<id>`, canvas `cv-<id>`.
Терраса и терраса у бассейна делят ОДИН редактор — `initRectCanvas(secId)`
...
Сделано в итерации v=159 (этап 1: высоты, разделы, кегль плана):

Первая группа правок из нового TODO (этап 1, пункты 1–5, 13, 14).

- **Минимумы высот — 15 см**, шаг слайдера у фундамента 10 → 5 см (`v-found`/`r-found`,
  `TERRACE_MIN_CM`, `TERRACE_MIN_H`). Потолок высоты настила по-прежнему = высота
  фундамента, так что «терраса не выше фундамента» держится само.
- **Кегль подписей на плане ×2**: все `ctx.font` в canvas.js переведены на `planFont()`
  с единым множителем `PLAN_FONT_K = 2` — правится в одном месте.
- **Кнопка «Дальше» на первом экране** появляется, только если дом уже выбран
  (`_dSyncSelectNext`): при возврате не нужно повторно кликать карточку, а повторный
  клик сбросил бы все настройки конструкций.
- **Навес над террасой убран совсем**: тумблер `railing-roof`, `buildTerraceCanopies`,
  `_terraceCanopyParams`, `_buildCanopySlab`, `threeState.canopyMeshes`, высокие
  столбы-опоры (`makeTallPost`) и рейкаст по низу плиты. Вместе с ними ушли
  `_insetWorldRect`, `_terraceColumnPoints`, `CANOPY_COL_SPACING` — они обслуживали
  только навес. Все столбы ограждения теперь одной высоты.
- **Причал убран совсем**: секция, редактор, `pierRects`/`activePierRect`, настил на
  отметке 0.5 м, зоны растительности, строка сметы и ветка `terraceHeight = 500`.
- **«Отделка фасада» скрыта из меню**, но код остался: у пункта появился флаг
  `hidden`, меню и замер ширины панели строятся по `D_MENU_ITEMS`.
- Cache-bust: `styles-desktop.css?v=31`, `state.js?v=49`, `canvas.js?v=47`,
  `viewer3d-core.js?v=137`, `viewer3d-railing.js?v=8`, `nav-desktop.js?v=87`.

Сделано в итерации v=158 (забор: серое по умолчанию, roughness 0.6):

- Правило материалов забора инвертировано: товаром красится только распознанное
  ПОЛОТНО (`FENCE_PANEL_RE`), все остальные части — тёмно-серые. Раньше искали
  каркас списком имён, и любая незнакомая деталь модели уезжала в текстуру товара.
- Каркасный материал: roughness 0.75 → **0.60** (по указанию продукта).
- `_fenceModelSection` возвращает число опознанных полотен; если по всей ломаной
  их 0 — `console.warn` с url модели: забор вышел целиком серым, надо дополнить
  `FENCE_PANEL_RE` именами из файла.
- `_fenceModelPost` (замыкающий столб) теперь выбрасывает из клона полотно, а не
  «оставляет каркас по списку» — то же правило, что и в секции.
- Cache-bust: `viewer3d-builders.js?v=24`.

Сделано в итерации v=157 (шов доски по середине проступи):

- Поперёк ступеней UV проступи теперь отсчитываются от её ЦЕНТРА (`uvAnchor` →
  третий аргумент `_applyDeckUV`), а не от мировой сетки: шов доски приходится
  ровно на середину ступени и одинаково на всех ступенях. Смещение получается
  порядка 0.1 м — как и просил продукт.
- Почему мировая проекция не годилась: доска ≈ 0.11 м, проступь 0.29 м, шаг
  лестницы 0.28 м. Периоды несоизмеримы, поэтому фиксированный сдвиг «на 0.1 м»
  ничего бы не гарантировал — шов всё равно гулял бы от ступени к ступени.
  Вдоль ступени проекция осталась мировой (масштаб рисунка общий с террасой).
- Проверено: у всех проступей v ∈ [−0.145, +0.145] — граница тайла (она же шов)
  ровно по центру; u остались мировыми.
- Cache-bust: `viewer3d-core.js?v=136`, `viewer3d-builders.js?v=23`.

Сделано в итерации v=156 (UV забора — из GLB):

- **Ориентация досок забора решена на стороне модели** (продукт, 2026-08-23): развёртка
  делается в GLB, фронт её не перепроецирует. Вопрос про признак ориентации в карточке
  товара (поле/тег/эвристика по имени) закрыт — ничего этого не нужно.
- В `_fenceModelSection` UV остаются авторскими; аварийная box-проекция применяется
  только к мешу, у которого UV вообще нет (иначе текстура товара легла бы одним пикселем).
- Столбы модели больше не попадают в `threeState.fenceMeshes`: по этому списку идёт
  примерка образца цветом, и она должна красить только полотно — то же правило, что и
  у материала товара.
- Cache-bust: `viewer3d-builders.js?v=22`.
...
- **Товаром красится только полотно.** Столбы и вспомогательные элементы — всегда
  тёмно-серые (`FENCE_FRAME_COLOR`). Значение занижено (0x2a2a2a): базовый цвет
  умножается на освещение, и «настоящий» тёмно-серый читался в кадре как средний
  (та же история, что с `PAD_COLOR`).
- **UV полотна — вдоль ДЛИННОЙ стороны панели**: обычная box-проекция даёт доски
  вдоль горизонтали, а для панели выше, чем шире, u/v меняются местами. Прежний
  безусловный разворот (всегда вертикальные доски) убран.
- **Модель товара больше не «съедает» материал.** `_fenceModelSection` брал материалы
  из GLB как есть, поэтому после загрузки модели забор возвращался к виду из файла —
  текстура товара пропадала. Теперь материалы назначаются по тому же правилу:
  меши с именем из `FENCE_FRAME_RE` (post/столб/каркас/…) — тёмно-серые, остальные —
  материал товара. Плюс `console.info('[fence] секции из модели товара: …')`, чтобы
  на стенде было видно, какая ветка сработала.
- Cache-bust: `viewer3d-builders.js?v=20`.

Сделано в итерации v=153 (ступени по тегу steps):

- Раздел ступеней (2330) получил тег `steps` и попал в `SECTION_TAG_ONLY`: по указанию
  бэкенда (2026-08-23) запрос должен идти БЕЗ `section_id` — `?tags=steps&limit=50`.
  Раньше уходил `?section_id=2330&limit=50` (тега у раздела не было вовсе), и товары
  не приходили — фронт показывал заглушки `STUB_RESULTS`.
- Cache-bust: `state.js?v=48`.

Сделано в итерации v=152 (текстура товара на условном заборе):

- **Условный забор берёт материал товара.** И полотно, и столбы красились константой
  `FENCE_SCHEMATIC_COLOR`, поэтому забор оставался серым даже когда у товара есть
  `texture_urls` (в каталоге превью при этом было). Теперь при выбранном товаре
  используется `M.deck` (уже разрешённый `_resolveDeckMat(_baseDeck, 'fence')`),
  без товара — прежний нейтральный цвет.
- **UV полотна**: box-проекция + swap u/v (`_applyFenceUV`), доски идут ВЕРТИКАЛЬНО —
  как у штакетника; у настила грувы по умолчанию горизонтальные. Накладывается
  только когда на материале есть карта.
- Модель товара (`glb_file_url`) по-прежнему в приоритете: есть GLB — строим секции
  из него со своими материалами, условный вид не используется.
- Cache-bust: `viewer3d-builders.js?v=19`.

Сделано в итерации v=151 (левая панель уже):

- Заголовок шага 3 — **«Благоустройство»** вместо «Элементы проекта» (короче, влезает
  в одну строку в узкой панели).
- **Ширина левой панели = заголовок + 32px с каждой стороны**, считается в рантайме
  (`_dSyncSidebarWidth`, nav-desktop.js) по РЕАЛЬНЫМ метрикам шрифта и записывается
  в `--sidebar-w`. Константой обойтись нельзя: одна и та же строка в Segoe UI и в
  запасных шрифтах отличается на 10-15%, и в константу пришлось бы закладывать запас.
  Пересчёт повторяется по `document.fonts.ready` — до загрузки шрифта метрики другие.
- Нижняя граница ширины — самая длинная строка меню (подпись 18px/700 + паддинги
  кнопки + рамки + зазоры + иконки правки/удаления + паддинг списка): уже неё панель
  срезала бы название элемента. Значение `--sidebar-w` в CSS осталось как фолбэк.
- Cache-bust: `styles-desktop.css?v=29`, `nav-desktop.js?v=86`.

Сделано в итерации v=150 (заборы на бэкенде, фолбэк каталога):

- Бэкенд выложил товары заборов под тегом `fences` (2026-08-23,
  `GET /api/v1/products/?tags=fences`). Фронт под этот тег уже настроен:
  `SECTION_TAGS[2348] = 'fences'`, раздел «Штакетник из ДПК».
- **Фолбэк каталога**: если запрос `section_id + тег` вернул ПУСТО, раздел
  перезапрашивается ТОЛЬКО по тегу. Товары тега могут лежать в других разделах —
  именно так было с мебелью, из-за чего появился `SECTION_TAG_ONLY`; теперь такой
  случай не оставляет раздел пустым (и не роняет его на заглушки), а лечится сам.
  Ошибка запроса по-прежнему даёт `null` (перезапросим позже), фолбэк на неё не срабатывает.
- Cache-bust: `nav-desktop.js?v=85`.
...
- **Пустая смета объясняет себя**: конструкции размечены, но ни один товар не применён —
  вместо таблицы из прочерков с итогом 0 ₽ выводится «Товары ещё не выбраны», и блок
  «Расчёт по спецификации» тоже не строится (расчёт на товарах по умолчанию рядом с этой
  фразой противоречил бы сам себе). Разметки нет вовсе — прежнее «Разметьте конструкции,
  чтобы рассчитать смету».
...
- Cache-bust: `nav-desktop.js?v=84`.
