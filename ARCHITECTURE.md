...
- **Тег `fencing`** (ручка бэкенда `GET /api/v1/products/?tags=fencing`, 2026-08-15) привязан
  к разделу 2331. Как и мебель, набор запрашивается **только по тегу** (`SECTION_TAG_ONLY`):
  ручка дана без `section_id`, и фильтр по разделу мог бы отрезать товары из соседних.
