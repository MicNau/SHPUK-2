...
| `deckingBoardProductId` | id доски: сначала выбор «В смету» (`S.estimate.terrace.id`), затем применённый товар (`S.elementMat.terrace.productId`) |
...
- **`S.elementMat[el]` теперь хранит `productId`/`name`** применённого товара — по нему
  расчёт узнаёт выбранную доску, если её не клали явно «В смету». 3D-слой эти поля не читает.
