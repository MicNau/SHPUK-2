...
**`mod_bench_a.glb` и `mod_lamp_a.glb` работают ещё и как ФОЛБЭК садовой мебели.**
Модели мебели приходят из каталога (поле товара `model_url` / `texture_urls.model_glb`);
пока бэкенд их не отдаёт, в точку встаёт локальная модель: светильник — если в названии
товара есть «лампа/светильник/фонарь/торшер», иначе скамья. См. `furnitureModelUrl`
в `viewer3d-builders.js` и раздел «Садовая мебель» в `ARCHITECTURE.md`.
